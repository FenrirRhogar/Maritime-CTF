...
print("="*40)
print("Starting simulation with TEAM-BASED strategies...")
print("BLUE TEAM (Agents 0, 1, 2) uses TRUTHFUL bidding.")
print("RED TEAM (Agents 3, 4, 5) uses BID SHADING (bidding 25% lower).")
print("="*40)
...
try:
    ...
    while True:
        ...
        all_bids = {}
        for agent in agents:
            # Red Team (Agents 3, 4, 5) uses the "shade" strategy
            if agent.agent_id in ["agent_3", "agent_4", "agent_5"]:
                bids = agent.get_bids(env, obs, info, n_steps=3, decay_factor=0.9, bidding_strategy='shade')
            else:
                # Blue Team (Agents 0, 1, 2) uses truthful bidding
                bids = agent.get_bids(env, obs, info, n_steps=3, decay_factor=0.9, bidding_strategy='truthful')
